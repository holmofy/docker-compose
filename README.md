...
Verify that the Clickhouse database has the same content:

```shell
docker-compose -f docker-compose.yaml exec clickhouse-server bash -c 'clickhouse-client -q "select * from inventory.customers"'
